...
public class Reflections {
	...
	public static Object invokeMethod(final Object obj, final Method method, final Object[] args) {
		...
		if (method == null) {
			throw new IllegalArgumentException("Could not find method [" + method.getName() + "] on target [" + obj + "]");
		}
			...
			for (Method method : methods) {
				if (method.getName().equals(methodName)) {
					...
					if(method.getParameterTypes().length==parameterTypes.length){
						if(compareParameter(method.getParameterTypes(),parameterTypes)){
							return method;
						}
					}
				}
	...
	private static boolean compareParameter(Class<?>[] fromParameterTypes, Class<?>[] toParameterTypes){
		boolean result=true;
		for(int i=0;i<fromParameterTypes.length;i++){
			Class<?> t=fromParameterTypes[i];
			Class<?> t1=toParameterTypes[i];
	...
	public static List getListField(List objectList, String field) {
		List<Object> result=new ArrayList<Object>();
		if(objectList!=null){
			for(Object obj:objectList){
				result.add(getFieldValue(obj, field));
			}
		}
